...
const TrialFloatingCta = () => {
  ...
  // under it — on a page this long that corner passes over review stars,
  // testimonial text, section headings and other buttons dozens of times per
  // scroll. Rather than trying to detect and dodge each one individually
  // (fragile, and still misses plain text), it hides for the whole time the
  // page is actively moving and only reappears once scrolling has stopped —
  // the same pattern mobile Safari's own toolbar uses. A user mid-scroll is
  // reading, not deciding; a user who has stopped is a fair moment to offer
  // the CTA, and by then whatever is under the pill is content they have
  // already scrolled past, not new text arriving.
  const [pastHero, setPastHero] = useState(false);
  const [settled, setSettled] = useState(true);

  useEffect(() => {
    const hero = document.getElementById("l3-home");
  ...
  return (
    <button
      type="button"
      className={`l3-floating-cta${shown ? " l3-floating-cta--in" : ""}`}
