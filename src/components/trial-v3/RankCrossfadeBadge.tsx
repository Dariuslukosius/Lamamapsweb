...
const RankCrossfadeBadge = ({ before, after }: RankCrossfadeBadgeProps) => {
  ...
  return (
    <button
      type="button"
      className="t3-rank-crossfade"
      onMouseEnter={() => setShowAfter(true)}
      onMouseLeave={() => setShowAfter(false)}
      onClick={() => setShowAfter((v) => !v)}
      aria-label={showAfter ? `After: ${after}` : `Before: ${before}`}
    >
      <span className={`t3-rank-crossfade-face${!showAfter ? " is-visible" : ""}`}>Before: {before}</span>
      <span className={`t3-rank-crossfade-face t3-rank-crossfade-face--after${showAfter ? " is-visible" : ""}`}>
        After: {after}
      </span>
    </button>
  );
